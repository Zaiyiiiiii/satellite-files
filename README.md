...
Files 是一个类似 [copyparty](https://github.com/9001/copyparty) 的文件服务器，面向**个人自部署**，界面简洁、现代。
...
`FILES_SECRET`、`FILES_MAX_UPLOAD`、`FILES_DEBUG` 如果在当前环境里设置了，`xmake run` 会原样传给组件。
...
wasmtime serve -Scli -Sp3 -Wcomponent-model-threading=y \
  --addr 127.0.0.1:8080 \
  --env FILES_TITLE="My Files" --env FILES_ACCOUNTS=alice:secret \
  --dir ./data::/mnt/data \
  target/wasm32-wasip3/release/files.wasm
```
...
| 环境变量 | 默认值 | 说明 |
|---|---|---|
| `FILES_TITLE` | `Files` | 页面上显示的站点名称 |
| `FILES_ACCOUNTS` | 未设置 | 账号列表：`alice:密码1,bob:密码2` |
| `FILES_ACCESS` | 见下文 | 按路径的访问规则 |
| `FILES_SECRET` | 由账号列表派生 | 会话 Cookie 的签名密钥。不设置时修改任意密码都会让所有会话失效 |
| `FILES_MAX_UPLOAD` | `0`（不限制） | 单个文件的上传上限（MiB） |
| `FILES_DEBUG` | 未设置 | 设置后会把每个请求打印到 stderr |

环境变量通过 `wasmtime serve --env KEY=VALUE` 传入。

#### 访问规则

`FILES_ACCESS` 由 `;` 分隔的规则组成，每条规则的格式是 `路径:对象=权限,对象=权限`。对某个路径生效的是**最长前缀匹配**的那条规则。
...
- `FILES_TITLE`、`FILES_ACCOUNTS`、`FILES_ACCESS`、`FILES_SECRET`、`FILES_MAX_UPLOAD`：同上表，不设置时用默认值（planet 把配置项按 `id` 注入成同名环境变量）
...
> 如果需要从其他机器访问，请先设置 `FILES_ACCOUNTS`。密码以明文保存在配置里，公网部署时请放在 HTTPS 反向代理后面。
...
`xmake e2e`（`xmake/modules/files/e2e.lua`）会用临时数据目录启动服务，然后用真实的 `curl` 和 `tar` 验证以下场景：
...
| `POST /.files/login` / `POST /.files/logout` / `GET /.files/me` | 会话相关 |
...
xmake/modules/files/   构建、运行时安装、e2e 测试、sysroot 重建
...
- 上传：请求体的 `stream<u8>` 经过计数管道（用于大小限制和完整性校验）交给 `descriptor.write-via-stream`，写入同目录下的 `.<name>.<random>.filespart`，成功后才 `rename-at` 到目标文件名
